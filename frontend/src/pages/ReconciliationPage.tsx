...
export function ReconciliationPage() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8">
      <h1 className="mb-6 page-title">{intl.formatMessage({ id: 'reconciliation.page.title' })}</h1>

      <section className="rounded-xl border border-slate-200 p-4 dark:border-slate-700 dark:bg-slate-900">
        <ExpenseForm />
      </section>
          ...
          <ul className="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-slate-800 dark:border-slate-700 dark:bg-slate-900">
            {expensesData.expenses.map((expense) => (
              <li key={expense.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <p className="shrink-0 text-sm font-semibold tabular-nums text-slate-800 dark:text-slate-200">
