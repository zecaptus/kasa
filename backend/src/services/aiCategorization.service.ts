...
type TransactionInput = Pick<ImportedTransaction, 'id' | 'label' | 'categorySource'>;
...
export function buildPrompt(
  ...
  transactions: Array<Pick<ImportedTransaction, 'label'>>,
): string {
  const categoryLines = categories.map((c) => `- "${c.id}" → ${c.name}`).join('\n');

  const transactionLines = transactions.map((t, i) => `${i}: "${t.label}"`).join('\n');
...
async function processBatch(
  ...
  for (const result of results) {
    ...
    if (outcome.categorized) categorized++;
